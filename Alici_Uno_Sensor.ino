...
RF24 radio(7, 8); // CSN-8, CE-7, SCK-13, MOSI-11, MISO-12 NRF24 pin bağlantıları.
...
int gaz=0;
int kornaPin = 5;// Işık çıkış pini.
int isikPin = 6;// Korna çıkış pini.
int joystick[7];// 7 kanallı joystik bilgisi değişkeni.
int SagtrigPin = 4; /* Sensorun Sagtrig pini Arduinonun 14 numaralı ayağına bağlandı */
int SagechoPin = 3;  /* Sensorun Sagecho pini Arduinonun 15 numaralı ayağına bağlandı */
int SoltrigPin = 2; /* Sensorun Soltrig pini Arduinonun 16 numaralı ayağına bağlandı */
int SolechoPin = 1;  /* Sensorun Solecho pini Arduinonun 17 numaralı ayağına bağlandı */
long Sagsure;
long Saguzaklik;
long Solsure;
long Soluzaklik;
